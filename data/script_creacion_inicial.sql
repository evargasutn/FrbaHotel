...
--/PROC GETRESERVA CANCELABLE
IF OBJECT_ID ( 'COMPUMUNDO_HIPER_MEGA_RED.getReservaCancelable', 'P' ) IS NOT NULL 
		DROP PROCEDURE COMPUMUNDO_HIPER_MEGA_RED.getReservaCancelable
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE PROCEDURE COMPUMUNDO_HIPER_MEGA_RED.getReservaCancelable
	@codReserva numeric(18)
	AS
	--SI RECIBE -1, MUESTRA TODOS LAS RESERVAS
	IF (@codReserva = -1)
		SELECT * FROM COMPUMUNDO_HIPER_MEGA_RED.RESERVAS R
		WHERE estado = 2 OR estado = 1			
	ELSE
		SELECT * FROM COMPUMUNDO_HIPER_MEGA_RED.RESERVAS R	
		WHERE estado = 2 OR estado = 1			
...
IF OBJECT_ID ( 'COMPUMUNDO_HIPER_MEGA_RED.getEstadoReserva', 'P' ) IS NOT NULL 
		DROP PROCEDURE COMPUMUNDO_HIPER_MEGA_RED.getEstadoReserva
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE PROCEDURE COMPUMUNDO_HIPER_MEGA_RED.getEstadoReserva
	@descripcion varchar(255)
	
	AS
	IF(@descripcion = '')
		SELECT * FROM COMPUMUNDO_HIPER_MEGA_RED.ESTADOS_RESERVA
	ELSE
		SELECT * FROM COMPUMUNDO_HIPER_MEGA_RED.ESTADOS_RESERVA
...
AS
	BEGIN
	RETURN (SELECT SUM(H.recargoEstrella * H.cantEstrellas) 
			FROM COMPUMUNDO_HIPER_MEGA_RED.DETALLES_RESERVA DR
...
AS
	BEGIN
		RETURN (SELECT DISTINCT TOP 1  TH.tipoPorcentual 
				FROM COMPUMUNDO_HIPER_MEGA_RED.DETALLES_RESERVA DR
...
AS
	...
	DECLARE @codConsumible numeric(18);
	...
	/*Step 3: Assign the query to the cursor.*/
	SET @Cursor_itemFactura = CURSOR FOR
		SELECT DISTINCT ITEMS.numeroItem, CE.codConsumible
		FROM COMPUMUNDO_HIPER_MEGA_RED.ITEMS_FACTURA ITEMS,
			 COMPUMUNDO_HIPER_MEGA_RED.CONSUMIBLES_X_ESTADIA CE,
			 COMPUMUNDO_HIPER_MEGA_RED.CONSUMIBLES C
		WHERE	ITEMS.numeroFactura = @numeroFactura
				AND CE.codReserva =  @codReserva
				AND ITEMS.descripcion = C.descripcion 
				AND CE.codConsumible = C.codConsumible
	...
	OPEN @Cursor_itemFactura
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				WHERE codReserva = @codReserva AND
					  codConsumible = @codConsumible
